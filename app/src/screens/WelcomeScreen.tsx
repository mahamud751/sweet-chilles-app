...
/** Top of form: sit below hero logo (tall asset ≈ 853×1844). */
const CONTENT_START_FRACTION = 0.38;
const CONTENT_PAD_TOP_MAX_FR = 0.44;
...
const WelcomeScreen = ({ onLoginSuccess }: WelcomeScreenProps) => {
  const { primary, brandName } = useBrandStyles();
    ...
    <SafeAreaView style={styles.safeRoot} edges={['bottom']}>
      ...
      <View style={styles.root}>
        <Image
          ...
          accessibilityLabel={`${brandName} welcome background`}
        />
        ...
        <View style={styles.contentLayer}>
          ...
          >
            <View style={styles.formContainer}>
              ...
              <TouchableOpacity
                style={[styles.mainBtn, { backgroundColor: primary }]}
                activeOpacity={0.8}
                disabled={loading}
                onPress={async () => {
                  setLoading(true);
                  ...
                  try {
                    ...
                    try {
                      session = unifiedAuthToSession(
                        await login(email.trim(), password),
                      );
                    } catch (e) {
                      if (e instanceof ApiError && e.status === 404) {
                        try {
                          const memberRes = await loginMember(
                            email.trim(),
                            password,
                            RESTAURANT_SLUG,
                          );
                          session = {
                            accountType: 'member',
                            token: memberRes.token,
                            member: memberRes.member,
                          };
                        } catch {
                          const staffRes = await loginStaff(
                            email.trim(),
                            password,
                          );
                          session = {
                    ...
                    const msg =
                      e instanceof ApiError
                        ? e.message
                        : 'Could not reach sweetsapi.savasuite.co.uk';
                    setError(msg);
                  } finally {
                    setLoading(false);
                  }
              ...
              >
                <Text style={styles.mainBtnText}>{loading ? 'Signing in…' : 'Log in'}</Text>
              </TouchableOpacity>
            </View>
            ...
            <Text style={styles.termsTxt}>
              ...
              <Text style={[styles.linkTxt, { color: primary }]}>Terms & Conditions</Text>{' '}
              and <Text style={[styles.linkTxt, { color: primary }]}>Privacy Policy</Text>
            </Text>
          </View>
        </View>
      </View>
